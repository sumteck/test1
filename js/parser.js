...
const TbrParser = (() => {
  ...
  function parseSparkBill(text) {
    ...
    // ── FIX 1: Bill Number (Strict Numeric/Hyphen matching)
    const billNoMatch = text.match(/Bill\s*(?:No|Number)[.:\s]+([0-9\-\/]+)/i);
    if (billNoMatch) {
      result.billNo = billNoMatch[1].trim();
    }
    ...
    // ── FIX 4: Net Amount (Handles table layout & Gross Salary fallback)
    const netSalMatch = text.match(/(?:Net\s*Sal|Net\s*Amount)[\s\S]*?([0-9][0-9,]*(?:\.[0-9]{1,2})?)(?:\s|$)/i);
    if (netSalMatch) {
        result.netAmount = parseAmount(netSalMatch[1]);
    }

    // Fallback: If amount is still 0, look for the last large number after "Gross Salary"
    if (!result.netAmount) {
        const grossToNetPattern = /Gross\s*Salary[\s\S]*?([0-9]{3,})/gi;
        let lastMatch;
        let match;
        while ((match = grossToNetPattern.exec(text)) !== null) {
            lastMatch = match[1];
        }
        if (lastMatch) {
            result.netAmount = parseAmount(lastMatch);
        }
    }
